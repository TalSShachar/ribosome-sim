...
class ThreePrimeSpliceSequenceCandidate:
    ...
    @staticmethod
    def _maybe_find_polypyrimidine_tract(three_prime_ss: str) -> pptc.PolyPyrimidineTractCandidate:
        if not three_prime_ss:
            return None

        if len(three_prime_ss) < pptc.MIN_LENGTH:
            return None

        return pptc.PolyPyrimidineTractCandidate(three_prime_ss)
    ...
    def get_length_score(self, intron_length_range: IntronLengthRange) -> float:
        ...
        average_intron_size_ratio = 1 - \
            ((delta - intron_length_range.min) / intron_length_range.len_range())
